...
vi.mock('recharts', () => ({
  ...
  LineChart: ({ children, data }: any) => (
    <div data-testid="line-chart" data-history={JSON.stringify(data)}>
      {children}
    </div>
  ),
...
describe('ActiveFoldersCard', () => {
  ...
  it('should render with initial data when no data is provided', () => {
    ...
    const numberDiv = screen.getByText('Pastas ativas').parentElement?.querySelector('.text-5xl')
  ...
  it('should render visualizar pastas button', () => {
    ...
    expect(button).toHaveClass('text-sm', 'font-medium', 'text-[#1CD6F4]', 'underline')
  })
